...
def plot_feature_importance(X, y, title):
    # Obliczamy F-score dla wszystkich cech
    selector = SelectKBest(f_classif, k='all')
    selector.fit(X, y)
    scores = selector.scores_
    
    # Sortujemy cechy od najlepszej
    indices = np.argsort(scores)[::-1]
    n_plot = min(20, X.shape[1]) # bierzemy top 20
...
def main():
    ...
    for label, data_file in [("set1", SET1_FILE), ("set2", SET2_FILE)]:
        print(f"\nAnaliza zbioru: {label}")
        X, y = load_dataset(data_file, LABELS_FILE)
        plot_feature_importance(X, y, label)
